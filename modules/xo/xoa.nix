...
let
  ...
  # Note: We use pkgs.nixoa.xen-orchestra-ce directly here since config values aren't fully
    ...
    let
      ...
      effectiveConfigSource =
        if cfg.configFile != null then
          ...
          tomlFormat.generate "xo-server-config.toml" cfg.settings
        else
          "${xoaPackage}/libexec/xen-orchestra/packages/xo-server/sample.config.toml";
    in
    {
      assertions = [
        {
      ...
      systemd.services.xo-server = {
        ...
        ++ (with pkgs; [
          ...
          coreutils
          nfs-utils
          cifs-utils # For NFS and SMB remote storage handlers
