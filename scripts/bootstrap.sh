...
write_vm_alias_settings() {
...

  {
    ...
    echo "{ ... }:"
    echo "{"
    echo "  vmHost = $(nix_quote "$hostname_arg");"
    echo "}"
  } > "$alias_file"
}
...
echo "  Branch: $branch"
echo "  Host directory: host/$hostname_arg"
...
echo "  Stable VM alias: vm -> ${hostname_arg}-vm"
...
template_dir="$repo_dir/host/_template"
host_dir="$repo_dir/host/$hostname_arg"
settings_file="$host_dir/_ctx/settings.nix"
hardware_file="$host_dir/_nixos/hardware-configuration.nix"
vm_alias_file="$repo_dir/host/_automation/default.nix"
...
echo "Created $host_dir from host/_template"

write_host_settings "$settings_file"
echo "Wrote $settings_file"

write_vm_alias_settings "$vm_alias_file"
echo "Updated $vm_alias_file"
...
git -C "$repo_dir" add "host/$hostname_arg" "host/_automation/default.nix"
echo "Staged host/$hostname_arg and host/_automation/default.nix so flake evaluation includes the new host"
...
echo "Stable VM alias: .#nixosConfigurations.vm -> .#nixosConfigurations.${hostname_arg}-vm"
echo "Next steps:"
echo "  1. Review $repo_dir/host/$hostname_arg/_ctx/settings.nix."
...
if [ "$first_switch" -eq 1 ]; then
  ...
  echo "  5. Future VM builds can use: nh os build $repo_dir#nixosConfigurations.vm"
else
  echo "  3. Run $repo_dir/scripts/apply-config.sh --hostname $hostname_arg"
  echo "  4. Build the stable VM target with: nh os build $repo_dir#nixosConfigurations.vm"
fi
