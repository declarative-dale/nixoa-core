...
nixoa_write_host_settings() {
  ...
  local enable_xen_hardware="false"
  local ssh_key=""

  if [ "$profile" = "vm" ]; then
    enable_xen_guest="true"
    enable_xen_hardware="true"
  fi

  {
    ...
    echo "  enableXenHardware = ${enable_xen_hardware};"
    echo ""
    echo "  systemPackages = [ ];"
    echo "  userPackages = [ ];"
    echo ""
    ...
    echo "    {"
    echo "      name = \"flathub\";"
    echo "      location = \"https://flathub.org/repo/flathub.flatpakrepo\";"
    echo "    }"
    echo "  ];"
    echo ""
    echo "  extraNixosModules = [ ];"
    echo "  extraNixosConfig = { };"
    echo "  extraHomeManagerModules = [ ];"
    echo ""
    echo "  immutability.enable = false;"
    echo ""
    echo "  xoConfig.toml = ''"
    echo "    [redis]"
    echo "    socket = \"/run/redis-xo/redis.sock\""
    echo ""
    echo "    [dataStore]"
    echo "    path = \"/var/lib/xo/data\""
    echo ""
    echo "    [tempDir]"
    echo "    path = \"/var/lib/xo/tmp\""
    echo ""
    echo "    [http]"
    ...
    echo "    [[http.listen]]"
    echo "    port = 443"
    echo "    cert = \"/etc/ssl/xo/certificate.pem\""
    echo "    key = \"/etc/ssl/xo/key.pem\""
    echo ""
    echo "    [http.mounts]"
    echo "    \"/\" = \"/var/lib/xo/xen-orchestra/@xen-orchestra/web/dist\""
    echo "    \"/v5\" = \"/var/lib/xo/xen-orchestra/packages/xo-web/dist\""
    echo "    \"/v6\" = \"/var/lib/xo/xen-orchestra/@xen-orchestra/web/dist\""
    echo ""
    echo "    [remoteOptions]"
    echo "    useSudo = true"
