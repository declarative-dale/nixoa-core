...
nixoa_append_first_install_nix_options() {
  ...
  command_ref+=(
    ...
    --option extra-substituters "$NIXOA_FLAKEHUB_SUBSTITUTER $NIXOA_XO_SUBSTITUTER $NIXOA_LIBVHDI_SUBSTITUTER"
    --option extra-trusted-public-keys "$NIXOA_FLAKEHUB_PUBLIC_KEY $NIXOA_XO_PUBLIC_KEY $NIXOA_LIBVHDI_PUBLIC_KEY"
  )
}
...
nixoa_write_host_settings() {
  ...
  local key
  {
    ...
    printf '    ];\n'
    printf '    enableExtras = false;\n'
    printf '    developmentMode = false;\n'
